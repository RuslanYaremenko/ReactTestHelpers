...
export const mountSmart = (component, store) => {
    const core = store
        ? <ThemeProvider theme={theme.lightTheme}><Provider store={store}>{component}</Provider></ThemeProvider>
...
export const renderSmart = (component, store) => {
    ...
    const renderedComponent = !store ? render(core)
        : render(
...
export const rerenderSmart = (component, store, rerender) => {
    ...
    const renderedComponent = !store ? rerender(core)
        : rerender(
            <Provider store={store}>
                {core}
            </Provider>);
...
const renderHelpers = component => ({
    ...
    rerenderSmart: (newComponent, store) => rerenderSmart(newComponent, store, component.rerender),
});
