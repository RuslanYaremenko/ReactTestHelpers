import { shallowSmart, mountSmart, mockStore } from "./testsHelper";
...
export const getMountWrapper = (component, { props = {}, store } = {}, debug) => {
    const mockedStore = store && mockStore(store);
    const wrapper = mountSmart(React.createElement(component, props), mockedStore).find(component);
...
export const sagasTests = (api, actions) => {
    return (saga, payload, returnedValue, { success, error } = {},
        { successName, errorName, requestName, noError, noActions,
            errorProvides = [], successProvides = [], requestArgs } = {}) => {
        ...
        describe(name, () => {
            ...
            it('should put SUCCESS', () => {
                let expected = expectSaga(saga, data)
                    .provide([
                        ...successProvides,
                        [matchers.call([api, api[requestName || name]], ..._.castArray(args)), returnedValue],
                        [matchers.call.fn(resolve)],
                    ]);
                if (!noActions) expected = expected.put(actions[successName || `${name}Success`](success));
                return expected.run();
            });
            if (!noError) { 
                it('should put ERROR', () => {
                    let expected = expectSaga(saga, data)
                        .provide([
                            ...errorProvides,
                            [matchers.call([api, api[requestName || name]], ..._.castArray(args)), throwError(new Error('error'))],
                        ]);
                    if (!noActions) expected = expected.put(actions[errorName || `${name}Error`](error));
